...
public class MyIntroduceActivity extends BaseActivity {
    ...
    protected void findViews() {
        ...
        et_sex.setText(getIntent().getStringExtra("introduce"));
        tv_title_right_text.setText("保存");
        tv_title_right_text.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(et_sex.getText().toString().equals("")){
                    Toast.makeText(getApplicationContext(),"请填写个人简介",Toast.LENGTH_SHORT).show();
                    return;
                }
                else{
                    showWaitingUI();
                    Api.alterUserIntroduce(et_sex.getText().toString(),alterNameHandler);

                    /*Intent intent = new Intent();
                    intent.putExtra("introduce", et_sex.getText().toString());// mCurrentDistrictName
