...
public class PhotoActivitys extends Activity {
	...
	public int max;
	public ArrayList<ForumPostsImage> url;
	public ArrayList<QuestionImages> urls;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		url= (ArrayList<ForumPostsImage>) getIntent().getSerializableExtra("URL");
		urls= (ArrayList<QuestionImages>) getIntent().getSerializableExtra("URLS");
		if(url!=null) {
			max = url.size();
		}
		if(urls!=null){
		...
		photo_bt_enter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for(int i=0;i<del.size();i++){				
					PhotoFileUtils.delFile(del.get(i)+".JPEG"); 
				}
				finish();
			}
		});
	...
	private void initListViews(int i) {
		...
		ImageLoader.getInstance().displayImage(url.get(i).getImageUrl(), img,options);
	...
	private void initListViewss(int i) {
		if (listViews == null)
			listViews = new ArrayList<View>();
		ImageView img = new ImageView(this);// 构造textView对象
		img.setBackgroundColor(0xff000000);
		//img.setImageBitmap(bm);
		ImageLoader.getInstance().displayImage(urls.get(i).getImageUrl(), img);
